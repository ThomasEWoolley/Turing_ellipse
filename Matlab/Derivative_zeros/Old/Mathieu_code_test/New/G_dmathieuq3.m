function csd = G_dmathieuq3(kf, m, q, xr, varargin)
...
%
%   This revised code improves accuracy for large q and xr values by using
%   an adaptive series length along with compensated summation to reduce
%   numerical error.
%
...
for vm = 1:lq
    if isreal(q(vm)) && real(q(vm)) < 0
        ...
        for k = 1:km(vm)
            ...
            % If desired, you can use variable-precision arithmetic here:
            % term = double(vpa(term, 50));
