function f2r=mmathieu2(kf,m,q,x,varargin)
%     ==============================================================
%     Purpose: Compute radial Mathieu functions of the second kinds Mcm(q,x)and Msm(q,x)
%     
...
fc(2)=q.*fc(3)./(a-4.0d0-2.0d0.*q.*q./(a+eps));
fc(1)=q./(a+eps).*fc(2);
...
for it=1 : 100;
mj=fix(fix(mj)+1);
x=x1-(x1-x0)./(1.0d0-f0./f1+eps);
[kd,m,q,x,mj,f]=cvf(fix(kd),fix(m),q,x,fix(mj),f);
if(abs(1.0-x1./(x+eps))<eps | f==0.0);
break;
end;
x0=x1;
f0=f1;
