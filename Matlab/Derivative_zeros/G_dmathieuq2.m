function csd = G_dmathieuq2(kf, m, q, xr, varargin)
% G_DMATHIEUQ3 Compute the derivative of angular Mathieu functions.
%
%   csd = G_dmathieuq3(kf, m, q, xr)
%   csd = G_dmathieuq3(kf, m, q, xr, 'MaxTerms', N, 'Tol', tol)
%
%   Inputs:
%       kf    - Function code:
...
%
%   This version has been modified to sort eigenvalues (ensuring proper
%   coefficient selection) and to disable early termination for high
%   parameter values (|q| or |xr| ≥ 30) to improve accuracy.
%
...
for k = 1:lq
    ...
    % For larger q, add extra terms for improved accuracy.
    if abs(q(k)) > 10
        km(k) = km(k) + 10;
    end
...
% Decide whether to disable early termination (for high |q| or |xr|)
disableBreak = (max(abs(q)) >= 15) || (abs(xr) >= 15);
...
%
%   This version has been modified to sort the eigenvalues so that the
%   correct eigenvector (and hence coefficients) is chosen.
%
%   Author: Da Ma (original), modified by [Your Name]
%   Date: [Date]
...
% Select the eigenvector based on the mode.
switch kd
    case 1
        idx = (m + 2) / 2;
    case {2,3}
